...
try: 
    ...
    print('cannot import pypixxlib module')
else:
    print('pypixxlib imported succesfully')
try:
    import psychopy
    from psychopy import visual, monitors
except ImportError:
    print('cannot import psychopy python module')
    PSYCHOPY_IS_IMPORTED = False
else:
    print('psychopy python module imported')
    PSYCHOPY_IS_IMPORTED = True

def get_dot_matrix(imSizePix, stimulusProportionFilled, dotSpacingPix, gridType): #This needs to make a circle of dots, not a square, 
    ...
    imDotCoordinatesYX = np.empty([imSizePix,imSizePix,2])
    imDotCoordinatesYX[:] = np.nan
...
def rotate_coordinates(expParameters,imDotCoordinatesYX):

    imDotCoordinatesUV = imDotCoordinatesYX.copy() 
    y = (imDotCoordinatesYX[:,:,0] - expParameters['imSizePix']/2)
    x = (imDotCoordinatesYX[:,:,1] - expParameters['imSizePix']/2)

    r = (x**2.0 + y**2.0)**0.5
    t = arctan2(y,x) + expParameters['globalStimulusRotationDeg'] * (pi/180)

    u = r * cos(t)
    v = r * sin(t)

    imDotCoordinatesUV[:,:,0] = u + expParameters['imSizePix']/2
    imDotCoordinatesUV[:,:,1] = v + expParameters['imSizePix']/2

    print(imDotCoordinatesYX.shape)
    print(imDotCoordinatesUV.shape)

    return imDotCoordinatesUV

def get_wavelet_positions(expParameters, checkSizeWavelets, isCheckPhaseReversed, gridType):
    imSizePix = int(expParameters['imSizePix'])
    im, imDotCoordinatesYX = get_dot_matrix(imSizePix, expParameters['stimulusProportionFilled'], expParameters['dotSpacingPix'], gridType)
    
    if expParameters['globalStimulusRotationDeg'] != 0:
        imDotCoordinatesYX = rotate_coordinates(expParameters,imDotCoordinatesYX)

    checkSizePix = expParameters['dotSpacingPix'] * checkSizeWavelets * 2
    ...
    if checkSizePix > 0:
        imMask = create_checkered_pattern(int(imSizePix), checkSizePix, isCheckPhaseReversed)
...
def make_coherence_stimulus(expParameters,
                            rng,
                            checkSizeWavelets,
                            isCheckPhaseReversed,
                            isFillBlankChecks,
                            oriDeg,
                            proportionCoherence,
                            weberContrast,
    ...
    yx = get_wavelet_positions(expParameters, checkSizeWavelets, isCheckPhaseReversed, gridType)
    ...
    if isFillBlankChecks and checkSizeWavelets > 0:
        ...
        blankSpaceFill = make_coherence_stimulus(expParameters, rng, checkSizeWavelets, not isCheckPhaseReversed, False, 0, 0, None, gridType)
        imC = imC + blankSpaceFill
        
    if weberContrast: # Normalize the image
        imC = (imC / numpymax(numpyabs(imC.flat))) * weberContrast
    if isModelMode == False:
        return imC
    if isModelMode == True:
        return waveletThetaDeg


def make_gaussian_stimulus(expParameters, rng, checkSizeWavelets, isCheckPhaseReversed, isFillBlankChecks, meanOriDeg, stdOriDeg, weberContrast, gridType):
    yx = get_wavelet_positions(expParameters, checkSizeWavelets, isCheckPhaseReversed, gridType)
    numWavelets = yx.shape[1]
...
def make_high_bit_depth_vpixx_window(doSetVideoMode=True):
 ...
 # Set the right video mode
  ...
  win = visual.Window(size=(1920, 1200), useFBO=True, fullscr=True, screen=1, gamma=1.92,
                      monitor=my_monitor, units='pix',weightBlanking=False)
